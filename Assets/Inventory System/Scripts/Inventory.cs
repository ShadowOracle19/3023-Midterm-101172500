...
public class Inventory : MonoBehaviour, ISaveHandler
{
    ...
    [SerializeField]
    public ItemTable[] recipe;

    [Tooltip("List size determines how many slots there will be. Contents will replaced by copies of the first element")]
    [SerializeField]
    public ItemTable recipeOutput;
    ...
    void Start()
    {
        ...
        for (int i = 0; i < craftingItems.Count && i < craftingSlots.Count; i++)
        {
            craftingSlots[i].SetContents(craftingItems[i], 8);
        }
    ...
    private void InitItemSlots()
    {
        ...
        for (int i = 1; i < craftingSlots.Count; i++)
        {
            ...
            ItemSlot newSlotCrafting = newObject.GetComponent<ItemSlot>();
            craftingSlots[i] = newSlotCrafting;
        }
    ...
    public Item checkRecipe()
    {
        ...
        for (int k = 0; k < recipe.Length; k++)
        {
            ...
            for (int i = 0; i < craftingSlots.Count; i++)
            {
                ...
                if (craftingSlots[i].ItemInSlot == recipe[k].GetItem(i))
                {
